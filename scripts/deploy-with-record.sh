...
if [[ "$WORKER_ONLY" -eq 1 ]]; then
  ...
  DASH_DIST="agent-dashboard/agent-dashboard/dist"
  ...
  ./scripts/with-cloudflare-env.sh npx wrangler r2 object put inneranimalmedia/dashboard/app/agent.html --file dashboard/agent.html --content-type "text/html" --config wrangler.production.toml --remote
