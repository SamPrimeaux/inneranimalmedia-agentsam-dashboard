...
"${WRAP[@]}" "${BUCKET_KEY_PREFIX}/agent.html" --file=dashboard/agent.html --content-type='text/html; charset=utf-8' --remote -c "$CONFIG"
"${WRAP[@]}" "${BUCKET_KEY_PREFIX}/agent-dashboard.js" --file=agent-dashboard/agent-dashboard/dist/agent-dashboard.js --content-type='application/javascript; charset=utf-8' --remote -c "$CONFIG"
"${WRAP[@]}" "${BUCKET_KEY_PREFIX}/agent-dashboard.css" --file=agent-dashboard/agent-dashboard/dist/agent-dashboard.css --content-type='text/css; charset=utf-8' --remote -c "$CONFIG"
