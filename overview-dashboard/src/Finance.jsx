...
function relDate(d) {
  if (d == null) return "—";
  const t = typeof d === "number" ? (d < 1e12 ? d * 1000 : d) : new Date(d).getTime();
...
export default function Finance(){
  ...
  const spendLedger=data?.spend_ledger||{ total:0, entries:0, by_provider:[] };
  const aiSpend=data?.ai_spend||{ total_usd:0, count:0, rows:[] };
  const financialHealth=data?.financial_health||{ total_in_all_time:0, total_out_all_time:0 };
      ...
      *{box-sizing:border-box;}input:focus,select:focus{border-color:rgba(74,222,128,0.5)!important;outline:none;}
      ...
      {tab==="overview"&&(
        <div style={{animation:"fadeIn 0.25s ease"}}>
          ...
          {/* Ledger & all-time health KPIs */}
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))",gap:14,marginBottom:20}}>
            {[
              {label:"Ledger Total",value:spendLedger.total,accent:"#a78bfa",sub:spendLedger.entries ? `${spendLedger.entries} entries` : null},
              {label:"AI / Usage Spend",value:aiSpend.total_usd,accent:"#06b6d4",sub:aiSpend.count ? `${aiSpend.count} line items` : null},
              {label:"Total In (All Time)",value:financialHealth.total_in_all_time,accent:"#4ade80"},
              {label:"Total Out (All Time)",value:financialHealth.total_out_all_time,accent:"#ef4444"},
            ].map(c=>(
              <div key={c.label} style={S.card}>
                <div style={S.lbl}>{c.label}</div>
                <div style={{...S.val,color:c.accent,fontSize:22}}>{fmt(c.value)}</div>
                {c.sub&&<div style={{fontSize:10,color:"rgba(255,255,255,0.4)",marginTop:4}}>{c.sub}</div>}
              </div>
            ))}
          </div>
          ...
          {/* Spend by provider (from spend_ledger) */}
          {spendLedger.by_provider?.length > 0 && (
            <div style={{...S.card,marginBottom:16}}>
              <div style={{...S.lbl,marginBottom:12}}>Infrastructure / AI Spend by Provider</div>
              <div style={{display:"flex",flexDirection:"column",gap:10}}>
                {spendLedger.by_provider.map((p,i)=>{
                  const pct = spendLedger.total ? (p.total / spendLedger.total) * 100 : 0;
                  return (
                    <div key={i}>
                      <div style={{display:"flex",justifyContent:"space-between",fontSize:11,color:"rgba(255,255,255,0.7)",marginBottom:4}}>
                        <span style={{textTransform:"capitalize"}}>{String(p.provider||"unknown").replace(/_/g," ")}</span>
                        <span style={{fontWeight:700,fontFamily:"'DM Mono',monospace"}}>{fmt(p.total)}</span>
                      </div>
                      <div style={{height:6,background:"rgba(255,255,255,0.06)",borderRadius:3,overflow:"hidden"}}>
                        <div style={{height:"100%",width:`${Math.min(pct,100)}%`,background:"#60a5fa",borderRadius:3,transition:"width 0.3s ease"}}/>
                      </div>
                    </div>
                  );
                })}
          ...
          {/* Recent AI / usage spend table */}
          {(aiSpend.rows?.length > 0 || aiSpend.total_usd > 0) && (
            <div style={S.card}>
              <div style={{...S.lbl,marginBottom:12}}>Recent AI / Usage Spend — {fmt(aiSpend.total_usd)} total</div>
              {aiSpend.rows?.length > 0 ? (
                <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
                  <thead><tr style={{borderBottom:"1px solid rgba(255,255,255,0.07)"}}>
                    {["Date","Provider","Description","Amount"].map(h=>(
                      <th key={h} style={{textAlign:h==="Amount"?"right":"left",fontSize:9,color:"rgba(255,255,255,0.35)",textTransform:"uppercase",letterSpacing:"0.08em",paddingBottom:8,fontWeight:700}}>{h}</th>
                    ))}
                  </tr></thead>
                  <tbody>
                    {aiSpend.rows.slice(0,20).map((r,i)=>(
                      <tr key={i} style={{borderBottom:"1px solid rgba(255,255,255,0.04)"}}>
                        <td style={{padding:"8px 0",color:"rgba(255,255,255,0.5)",fontFamily:"'DM Mono',monospace",whiteSpace:"nowrap"}}>{r.occurred_at ? relDate(r.occurred_at) : "—"}</td>
                        <td style={{padding:"8px 8px 8px 0",color:"rgba(255,255,255,0.75)"}}>{r.provider_slug || r.provider || "—"}</td>
                        <td style={{padding:8,maxWidth:220,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",color:"rgba(255,255,255,0.65)"}} title={r.description||r.notes}>{r.description||r.notes||"—"}</td>
                        <td style={{padding:"8px 0",fontWeight:700,textAlign:"right",fontFamily:"'DM Mono',monospace",color:"#06b6d4"}}>{fmt(r.amount_usd)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ) : (
                <div style={{padding:16,textAlign:"center",color:"rgba(255,255,255,0.4)",fontSize:13}}>No line items; total {fmt(aiSpend.total_usd)} from ledger.</div>
              )}
            </div>
          )}
        </div>
