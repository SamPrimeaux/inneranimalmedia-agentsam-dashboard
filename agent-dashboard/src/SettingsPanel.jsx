...
function ProviderDocsTab({ onOpenGeneral }) {
  ...
  useEffect(() => {
    let cancelled = false;
    setLoading(true);
    fetch("/api/settings/docs-providers", { credentials: "same-origin" })
      .then((r) => r.json().then((d) => ({ ok: r.ok, d })))
      .then(({ ok, d }) => {
        if (!ok) throw new Error(d?.error || "Failed to load providers");
        if (!cancelled) setProviders(Array.isArray(d.providers) ? d.providers : []);
      })
      .catch((e) => {
        if (!cancelled) {
          setError(e?.message || String(e));
  ...
  return (
    <div style={{ padding: 16, overflowY: "auto", flex: 1 }}>
      {loading ? (
        ...
        <div style={{ fontSize: 11, color: "var(--text-muted)", padding: 20, textAlign: "center" }}>No provider docs.</div>
      ) : (
        <div style={{
          display: "grid",
        ...
        >
          {providers.map((p) => (
            ...
            >
              <div style={{ fontWeight: 600, fontSize: 13, color: "var(--text-primary)", marginBottom: 8 }}>{p.label}</div>
              {p.blurb ? (
                <div style={{ fontSize: 11, color: "var(--text-secondary)", lineHeight: 1.55, marginBottom: 10 }}>{p.blurb}</div>
              ) : null}
              <div style={{ display: "flex", flexDirection: "column", gap: 6, fontSize: 11 }}>
                {p.docs_url ? (
                  <a href={p.docs_url} target="_blank" rel="noopener noreferrer" style={{ color: "var(--accent)", textDecoration: "none" }}>Documentation</a>
                ) : null}
                {p.api_url ? (
                  <a href={p.api_url} target="_blank" rel="noopener noreferrer" style={{ color: "var(--accent)", textDecoration: "none" }}>API Console</a>
                ) : null}
                {p.vault_key ? (
                  <button
                    type="button"
...
export default function SettingsPanel({
  ...
  const tabContent = {
    ...
    provider_docs: <ProviderDocsTab onOpenGeneral={() => pickTab("general")} />,
  };
